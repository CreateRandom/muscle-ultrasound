...
from baselines import get_brightness_factor, get_lr_model, get_mapped_path, evaluate_roc, find_threshold_for_specificity
from deployment.conditions import trained_on_esaote, conditions_to_run
from deployment.load_multi_input import load_checkpoint, load_multi_input, load_image_baseline, load_transform_dict, \
...
def test_checkpoint(condition, checkpoint_dict, adjustment, source, target, set_name, test_run=False):
    ...
    #  target = 'Philips_iU22'
    #  source = 'ESAOTE_6100'  # 'GE_Logiq_E_im_muscle_chart'
    #  set_name = 'test'  # can specify 'test' or 'val
    eval_set = target + '_' + set_name  # 'GE_Logiq_E_im_muscle_chart'# 'ESAOTE_6100_val'

    set_spec_dict = get_default_set_spec_dict(local=True)
    set_spec = set_spec_dict[eval_set]
    # source = set_spec_dict[checkpoint_dict['config']['source_train']].device
    # target = set_spec.device
...
def main():
    ...
    # evaluate various adjustments
    adjustments_e_and_p = ['mapped_images']# ['','regression', 'brightness']#, 'mapped_images']
...
#  base_checkpoint_path = '/mnt/chansey/klaus/muscle-ultrasound/checkpoints/'
   #
   #  project_name = 'createrandom/mus-coral'
   #  experiment = 'CORAL-133'
   #  file_name = 'pref_checkpoint'
   #  epoch = 5
   #  adjustment = None#'mapped_images' #'regression'
   ...
   #  target = 'Philips_iU22'
   #  source = 'ESAOTE_6100' # 'GE_Logiq_E_im_muscle_chart'
   #  evaluate_on = 'test' # can specify 'test' or 'val
   #  eval_set = target + '_' + evaluate_on #'GE_Logiq_E_im_muscle_chart'# 'ESAOTE_6100_val'
   #
   #  set_spec_dict = get_default_set_spec_dict(local=True)
   #  set_spec = set_spec_dict[eval_set]
   # # source = set_spec_dict[checkpoint_dict['config']['source_train']].device
   ...
   #                        attribute_specs=[att_spec, muscle_spec, side_spec], transform=transform,
   #                        use_pseudopatients=False,
   #                        return_attribute_dict=True, strip_folder=strip_folder, enforce_all_images_exist=enforce_all_images_exist)
   #
   #  loader = wrap_in_bag_loader(ds, 1, pin_memory=True, return_attribute_dict=True, shuffle=False)
   #
   #  attention_ei_frames = []
   ...
   #          if 'attention_outputs' in pred_output:
   #              att_scores = pred_output['attention_outputs'][0].numpy().tolist()[0]
   #              att_frame = pd.DataFrame({'Side': sides, 'Muscle': muscles,'Score': att_scores})
   #
   #              total = pd.merge(att_frame, ei_frame, on=['Muscle', 'Side'])
   #              attention_ei_frames.append(total)
   #
   #          if 'image_preds' in pred_output:
   #              image_scores = pred_output['image_preds'][0].flatten().numpy().tolist()
   #              im_frame = pd.DataFrame({'Side': sides, 'Muscle': muscles, 'Score': image_scores})
   #              total = pd.merge(im_frame, ei_frame, on=['Muscle', 'Side'])
   #              image_ei_frames.append(total)
   #  score_frame = None
   #  if attention_ei_frames:
   ...
   #  pred_path = os.path.join(export_path,'proba')
   #  os.makedirs(pred_path, exist_ok=True)
   #  exp_name = experiment.replace("/","_") + '_' + str(epoch)
   #  adjustment_name = adjustment if adjustment else ''
   #  csv_path = os.path.join(pred_path,(exp_name + '_' + adjustment_name + '.csv'))
   #  pd.DataFrame(y_pred, columns=['pred']).to_csv(csv_path, index=False, header=True)
   #
