...
from inference.load_multi_input import load_multi_input, load_checkpoint, load_transform_from_checkpoint
...
class ModelReadoutWrapper(nn.Module):
    ...
    def format_readout(self):
        pred_dict = {}
        # would nice to have: adjust behaviour for different heads based on type
...
if __name__ == '__main__':
    ...
    if not os.path.exists(base_path):
        print("No input folder present, nothing to do.", file=sys.stdout)
        sys.exit(0)

    file_names = [fn for fn in os.listdir(base_path) if str.endswith(fn, extensions)]

    if not file_names:
        print("No files were found in the input, nothing to do.", file=sys.stdout)
        sys.exit(0)
 ...
 #   file_name = file_name + '_' + str(epoch) + '.pt'
    ...
    checkpoint_dir = 'MIL-28_7.pt'

    # check if cuda is available
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    checkpoint = load_checkpoint(checkpoint_dir,device=device)
    ...
    for fn in file_names:
        ...
        # adjust image readout depending on the number of channels
